      ...
      try {
        ...
        if (!res.ok) {
          ...
          return {
            ...
            success: false,
            output: '',
            error: `Supabase error ${res.status}: ${err}`,
            durationMs: Date.now() - start,
          };
        }
  ...
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      if (!tableName || typeof tableName !== 'string') {
        return {
          toolName: 'supabase_create_table',
          success: false,
          output: '',
      ...
      if (!columnsRaw || typeof columnsRaw !== 'string') {
        return {
          ...
          success: false,
          output: '',
          error: 'columns must be a non-empty JSON string',
          durationMs: Date.now() - start,
        };
      }
      ...
      try {
        ...
        return {
          ...
          success: false,
          output: '',
          error: 'columns must be valid JSON',
          durationMs: Date.now() - start,
        };
      }
      if (!Array.isArray(columns) || columns.length === 0) {
        return {
          ...
          success: false,
          output: '',
          error: 'columns must be a non-empty JSON array',
          durationMs: Date.now() - start,
        };
      }
      ...
      try {
        ...
        return {
          toolName: 'supabase_create_table',
          success: true,
          output: `Table "${schema}"."${tableName}" created successfully`,
          durationMs: Date.now() - start,
        };
      } catch (e) {
        clearTimeout(timeout);
...
const supabaseManageRLSTool: Tool = {
  definition: {
    ...
    params: [
      { name: 'table_name', type: 'string', description: 'Name of the table', required: true },
      {
        name: 'action',
        type: 'string',
        description: 'RLS action to perform',
        required: true,
  ...
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      if (
        !action || typeof action !== 'string' || !['list', 'enable', 'disable'].includes(action)
      ) {
        return {
          toolName: 'supabase_manage_rls',
          success: false,
          output: '',
          error: 'action must be one of: list, enable, disable',
          durationMs: Date.now() - start,
        };
      }
      let query: string;
      switch (action) {
        case 'list':
          query =
            `SELECT policyname, cmd, qual, with_check FROM pg_policies WHERE tablename = '${tableName}'`;
          break;
        case 'enable':
      ...
      try {
        ...
        return {
          toolName: 'supabase_manage_rls',
          success: true,
          output: `RLS ${action === 'enable' ? 'enabled' : 'disabled'} for table "${tableName}"`,
          durationMs: Date.now() - start,
        };
      } catch (e) {
        clearTimeout(timeout);
  ...
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      if (!name || typeof name !== 'string') {
        return {
          toolName: 'supabase_deploy_function',
          success: false,
          output: '',
    ...
    params: [
      {
        ...
        required: true,
        options: ['list_buckets', 'create_bucket', 'upload_file'],
      },
      {
        name: 'bucket_name',
        type: 'string',
  ...
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      if (
        ...
        !['list_buckets', 'create_bucket', 'upload_file'].includes(action)
      ) {
        return {
          toolName: 'supabase_manage_storage',
          success: false,
          output: '',
          error: 'action must be one of: list_buckets, create_bucket, upload_file',
          durationMs: Date.now() - start,
        };
      }
      const base = config.supabaseUrl;
          ...
          case 'create_bucket': {
            ...
            if (!bucketName || typeof bucketName !== 'string') {
              ...
              return {
                ...
                success: false,
                output: '',
                error: 'bucket_name is required for create_bucket',
                durationMs: Date.now() - start,
              };
            }
            url = `${base}/storage/v1/bucket`;
          ...
          case 'upload_file': {
            ...
            if (!bucketName || typeof bucketName !== 'string') {
              ...
              return {
                ...
                success: false,
                output: '',
                error: 'bucket_name is required for upload_file',
                durationMs: Date.now() - start,
              };
            }
            ...
            if (!fileContent || typeof fileContent !== 'string') {
              ...
              return {
                ...
                success: false,
                output: '',
                error: 'file_content is required for upload_file',
                durationMs: Date.now() - start,
              };
            }
            url = `${base}/storage/v1/object/${encodeURIComponent(bucketName)}/${
